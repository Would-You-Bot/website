...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const typeKey = (parseInt(req.query.type as string) || 0).toString();
  const packType = types[typeKey].toLowerCase();
  const unreviewed = Boolean(req.query.unreviewed);
  const perPage = 9;

  const searchQuery = req.query.search as string | undefined;
  const queryCondition: any = unreviewed
    ? { reviewed: false }
    : { reviewed: true };
    queryCondition.type = { $regex: `^${packType}$`, $options: "i" };
  ...
  try {
    ...
    const packs = await Pack.find(queryCondition).skip(skip).limit(perPage);
